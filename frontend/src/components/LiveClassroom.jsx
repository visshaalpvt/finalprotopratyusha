    ...
    <div className="relative flex flex-col h-[calc(100vh-4rem)] bg-meeting overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className={cn(
          ...
          {/* Teacher Speech Recognition Controls */}
          {amITeacher && speechSupported && (
            <div className="mb-3 flex items-center gap-3 px-4 py-2.5 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10">
              <button
                onClick={isListening ? stopListening : startListening}
                className={cn(
                  "px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest transition-all flex items-center gap-2",
                  isListening 
              ...
              >
                {isListening ? (
                  <><span className="w-2 h-2 bg-white rounded-full animate-ping" /> RECORDING</>
                ) : (
                  <><span className="w-2 h-2 bg-emerald-900 rounded-full" /> START CAPTIONS</>
                )}
              </button>
              <span className="text-slate-400 text-xs">
                {isListening ? 'Speech is being captured for live captions' : 'Click to start live captioning'}
              </span>
            </div>
          )}
          ...
          {layout === 'speaker' ? (
            <div className="h-full flex flex-col gap-4">
              ...
              {showCaptions && transcript.length > 0 && (
                <div className="w-full max-w-4xl mx-auto px-6 pointer-events-none">
                  <motion.div 
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    className="bg-black/80 backdrop-blur-2xl px-8 py-4 rounded-3xl border border-white/10 shadow-3xl text-center"
                  >
              ...
              <div className="h-32 sm:h-36 flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                {!amITeacher && (
        ...
        <AnimatePresence>
          {activePanel === 'accessibility' && (
            <motion.div
              ...
              className="fixed right-4 top-20 bottom-24 w-[320px] bg-slate-900/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl z-40 flex flex-col overflow-hidden"
            >
              <div className="p-4 border-b border-white/5 flex items-center justify-between">
                <h3 className="text-white font-bold flex items-center gap-2">✨ Accessibility & Tools</h3>
